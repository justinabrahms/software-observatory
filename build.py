...
def generate_atlas_page(sensors, output_dir):
    ...
    # Build the matrix: rows = families, columns = stack layers
    # Each cell shows sensor chips that belong to that family AND that stack level

    # First, build the matrix grid
    matrix_rows = ""
    for family in FAMILIES:
        fam_sensors = by_family.get(family["slug"], [])
...
"""
        ...
        for layer in atlas_layers:
            ...
            for s in fam_sensors:
                ...
                if s.get("stack_level") == layer["slug"]:
  ...
  <div class="atlas-container">
    <div class="atlas-intro">
      <p style="color:var(--text-soft);max-width:44rem;line-height:1.7;">
        ...
        asking?</em> Find the row. <em>What level of evidence do you
        need?</em> Find the column. The cell points you to the right sensors.
        Empty cells are opportunities — places where no sensor exists yet.
      </p>
    </div>
...
{matrix_rows.rstrip()}
    ...
    <div class="atlas-axes">
      ...
      <div class="axis-label-v">← cheaper / faster &nbsp;&nbsp;|&nbsp;&nbsp; more expensive / more meaningful → <br>Confidence stack layers</div>
    </div>
