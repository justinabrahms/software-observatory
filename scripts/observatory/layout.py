"""The shared HTML shell: <head>, header, footer, and the page wrapper.

Every generated page is html_page(title, body) — one place that owns the
document skeleton, the nav, the canonical URL and the meta tags, so a change
to the chrome cannot land on eleven pages and miss the twelfth."""

import html

from .config import SITE_URL
from .jsonld import CONTENT_LICENSE, json_ld_script
from .render import add_heading_ids, meta_description
...
<head>
  ...
  <!-- The catalog content is CC BY-SA 4.0. rel="license" is the machine-
       readable half of that; the footer badge is the human half, and one
       without the other is how content gets reused with the attribution
       silently dropped. -->
  <link rel="license" href="{CONTENT_LICENSE}">
