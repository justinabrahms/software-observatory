...
- **[Challenge the taxonomy](https://github.com/justinabrahms/software-observatory/issues/new?template=taxonomy.yml)**
  ...
  in `scripts/observatory/taxonomy.py` and the `--fam-<slug>` color tokens in
...
order CI runs them — the site generator's snapshot suite
(`scripts/test_build.py`), frontmatter and vocabulary validation
...
### Changing how the site renders

`scripts/observatory/` is the entire CMS. `make test` builds the five-entry
fixture catalog in `tests/fixtures/content/` into a temp directory and asserts
named properties of the result: two builds of the same content are
byte-for-byte identical; a bad `family`, an unresolvable `see_also` or an
unknown `stack_level` fails the build *before* it writes a file; payloads are
escaped everywhere they land; every sensor gets a page; the CLI and web
datasets agree.

If you change a renderer, see what actually moved before you open the PR:

```sh
make diff-build            # last commit vs working tree, on the real catalog
make diff-build BASE=main  # or any rev
```

It prints the files that changed and writes the full unified diff to a
scratch file. Read it; a change that moves cells on the atlas should move
exactly the cells you meant. Nothing is committed and nothing needs
re-blessing.

Fixtures are written by hand and are deliberately fake (`SO-T01`… `Alpha
Probe`). Do not copy real entries into `tests/fixtures/` — a fixture that
changes whenever the catalog changes is not a fixture. Add a fixture entry
when you add a *rendering path* the current five do not reach.
...
- **Python in `scripts/observatory/`:** 4-space indent, section banners like
