...
`scripts/observatory/` is the entire CMS, and it is covered by golden-file
snapshot tests: `make test` builds the five-entry fixture catalog in
`tests/fixtures/content/` into a temp directory and diffs every generated byte
against the committed snapshot in `tests/golden/`. It also asserts that two
builds of the same content are byte-for-byte identical, and that a bad
`family`, an unresolvable `see_also` or an unknown `stack_level` fails the
build *before* it writes a file.

If you change a renderer, `make check` will fail with a diff of exactly which
generated bytes moved. **Read that diff — it is the review artifact.** A PR
that moves cells on the atlas shows you which cells moved. When the change is
intentional:

```sh
make test-update      # re-bless tests/golden/
git diff tests/golden/
```

Commit the re-blessed goldens **in the same commit as the rendering change**.
A goldens-only commit is unreviewable, and a rendering change without them
leaves `main` red.
...
when you add a *rendering path* the current five do not reach, and re-bless.
