.PHONY: build check check-frontmatter check-links check-citations check-external \
        check-deploy check-caddy cli-test test diff-build review-queue deploy serve
...
# Build tests: builds tests/fixtures/content into a temp directory, asserts
# the build is byte-for-byte deterministic, asserts each input gate fails
# before writing a single file, and checks escaping, page structure and
# dataset parity. Stdlib unittest only -- no pytest, nothing added to the
# pinned requirements.txt, one command, like cli/test/smoke.mjs.
test:
	$(PY) scripts/test_build.py

# Before/after diff of the generated site for a renderer change: builds BASE
# (default HEAD) and the working tree into scratch directories on the real
# catalog and reports what moved. On demand, nothing committed, not a gate.
BASE ?= HEAD
diff-build:
	$(PY) scripts/diff_build.py $(BASE)
...
# five-entry fake catalog that would read as real entries if it reached the
# web root, and it is not content.
#
